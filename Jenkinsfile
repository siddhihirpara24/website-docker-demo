pipeline {  
    ...
    environment {  
        ...
        AWS_ACCOUNT_ID = '136455701489'  
        IMAGE_TAG = "${env.BUILD_NUMBER}"  
        IMAGE_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:${IMAGE_TAG}"  
        LATEST_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:latest"  
        DEPLOY_SERVER = '3.111.245.44'  
    }  
  
    stages {  
        stage('Checkout') {  
            steps {  
                git branch: 'main', url: 'https://github.com/siddhihirpara24/website-docker-demo.git'  
            }  
        }  
        ...
        stage('Login to ECR') {  
            steps {  
                sh '''  
                    ...
                    136455701489.dkr.ecr.$AWS_REGION.amazonaws.com  
                '''  
            }  
        }  
